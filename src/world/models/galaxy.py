...
class StarGenerator():
    ...
    def rndSolar(self, num_planets=0, blue_sun=False, only_blue=False):
        import random
        earthPlanets = [self.EarthPlanet(i + 1) for i in range(40)]
        layerPlanets = [self.LayerPlanet(i + 1) for i in range(40)]
        terplanets = [self.TerPlanet(i + 1) for i in range(40)]
        moonPlanets = [self.MoonPlanet(i + 1) for i in range(40)]
        gasPlanets = [self.GasPlanet(i + 1) for i in range(40)]
        ...
        for i in range(nrPlanets):
            rnMargin = (random.random() * self.margin - 0.9 + 1) + 0.9
            if i < 6:
                if not earth:
            ...
            if planet_type.earth:
                dayType = 1
                orbitType = 1
                atmospheres=self.atmospheres[1:]
            else:
            ...
            planet = Planet(
                ...
                environment=random.choice(self.environment),
                surface_map=random.choice(self.map),
